...
export default function AuditPage() {
  ...
  const audit = snapshot.audit.slice().reverse();
  ...
  const searchActor = filters.actor.toLowerCase();
  const filtered = useMemo(() => {
    return audit.filter((entry) => {
      if (filters.entity && !entry.entity.includes(filters.entity))
        return false;
      if (filters.actor) {
        const actorName =
          usersById.get(entry.actor)?.toLowerCase() ??
          entry.actor.toLowerCase();
        if (
          !actorName.includes(searchActor) &&
          !entry.actor.toLowerCase().includes(searchActor)
        ) {
          return false;
        }
      }
      if (
        filters.from &&
        new Date(entry.at).getTime() < new Date(filters.from).getTime()
      )
        return false;
      if (
        filters.to &&
        new Date(entry.at).getTime() > new Date(filters.to).getTime()
      )
        return false;
      return true;
    });
  }, [audit, filters.entity, filters.from, filters.to, usersById, searchActor]);
  ...
  return (
    <div className="space-y-3">
      ...
      <PageIntro>
        Lacak aktivitas penting sistem untuk investigasi atau kebutuhan audit
        internal.
      </PageIntro>
      <div className="bg-white border rounded p-3 space-y-3">
        ...
        <button className="px-3 py-1 border rounded" onClick={exportJson}>
          Export JSON
        </button>
            ...
            <tbody>
              {filtered.map((entry, i) => (
                <tr key={`${entry.at}-${i}`}>
                  ...
                  <td className="p-2">
                    {usersById.get(entry.actor) ?? entry.actor}
                  </td>
