...
export default function BatchesPage() {
  ...
  function exportBatch(batchId: string, type: "csv" | "json") {
    ...
    if (!batch) return;
    const data = batch.items.map((id) =>
      snapshot.applications.find((a) => a.id === id),
    );
    ...
    if (type === "json") {
      ...
      const rows = data
        .map((a) =>
          a ? `${a.id},"${a.applicant.name}",${a.status},${a.assigned_to}` : "",
        )
        .join("\n");
    ...
    <div className="space-y-4">
      ...
      {loadError && (
        <div className="bg-rose-50 border border-rose-200 text-rose-700 text-sm rounded p-3">
          Gagal memuat batch terbaru: {loadError}
        </div>
      )}
      <section
      ...
      >
        ...
        <div className="flex flex-wrap gap-2">
          {readyApps.map((app) => (
            <label
              key={app.id}
              className="text-xs border rounded px-2 py-1 flex flex-col"
            >
      ...
      >
        ...
        <div className="space-y-3">
              ...
              >
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">
                      ID {batch.id} · {batch.items.length} items
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <RoleGate allow={["ADMIN"]}>
                    <button
                      className="px-3 py-1 border rounded"
                      onClick={() => exportBatch(batch.id, "json")}
                    >
                      Export JSON
                    </button>
                  </RoleGate>
                    ...
                    <ul className="space-y-1 text-xs">
                      ...
                      {batch.items.length === 0 && (
                        <li className="italic text-slate-500">
                          Belum ada penerima di batch ini.
                        </li>
                      )}
                    </ul>
                  </div>
          ...
          {snapshot.batches.length === 0 && (
            <p className="text-sm text-slate-500">Belum ada batch dibuat.</p>
          )}
        </div>
      </section>
    </div>
