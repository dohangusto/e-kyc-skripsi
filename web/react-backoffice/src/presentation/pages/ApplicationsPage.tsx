...
export default function ApplicationsPage() {
  const [filters, setFilters] = useState<Filters>(() =>
    parseParams(new URLSearchParams(window.location.search)),
  );
  const [simulate, setSimulate] = useState<"normal" | "empty" | "error">(
    "normal",
  );
  ...
  const usersById = useMemo(() => {
    ...
    db.users.forEach((u) => map.set(u.id, u.name || u.id));
    return map;
  }, [db.users]);
  ...
  const rows = useMemo(() => {
    ...
    if (filters.statuses.length)
      r = r.filter((a) => filters.statuses.includes(a.status));
    if (filters.region) {
      r = r.filter(
        (a) => `${a.region.kab} / ${a.region.kec}` === filters.region,
      );
    }
    if (filters.flaggedOnly) {
      r = r.filter(
        (a) =>
          a.flags.duplicate_face ||
          a.flags.duplicate_nik ||
          a.flags.device_anomaly,
      );
    }
    if (filters.assigned)
      r = r.filter((a) => a.assigned_to === filters.assigned);
    if (filters.scoreMin !== null)
      r = r.filter(
        (a) =>
          a.scores.ocr >= filters.scoreMin || a.scores.face >= filters.scoreMin,
      );
    if (filters.scoreMax !== null)
      r = r.filter(
        (a) =>
          a.scores.ocr <= filters.scoreMax && a.scores.face <= filters.scoreMax,
      );
    if (filters.dateFrom) {
    ...
    if (filters.search) {
      ...
      r = r.filter(
        (a) =>
          ...
          a.applicant.nik_mask.includes(s),
      );
    }
    ...
    <div className="space-y-4" aria-live="polite">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            className={`px-3 py-1 border rounded ${simulate === "normal" ? "bg-slate-200" : ""}`}
            onClick={() => setSimulate("normal")}
          >
            Normal
          </button>
          <button
            className={`px-3 py-1 border rounded ${simulate === "empty" ? "bg-slate-200" : ""}`}
            onClick={() => setSimulate("empty")}
          >
            Simulate Empty
          </button>
          <button
            className={`px-3 py-1 border rounded ${simulate === "error" ? "bg-slate-200" : ""}`}
            onClick={() => setSimulate("error")}
          >
            Simulate Error
          </button>
        </div>
      ...
      <PageIntro>
        Kelola dan filter semua pengajuan e-KYC untuk menentukan prioritas
        tindak lanjut.
      </PageIntro>

      <section
        className="bg-white border rounded p-3 space-y-3"
        aria-label="Filters"
      >
        <div className="grid md:grid-cols-3 gap-3">
          ...
          <label className="text-sm flex flex-col gap-1">
            ...
            <select
              ...
              onChange={(e) =>
                setFilters((f) => ({ ...f, assigned: e.target.value }))
              }
            >
        ...
        <div className="grid md:grid-cols-2 gap-3">
          ...
          <label className="text-sm flex flex-col gap-1">
            ...
            <input
              ...
              min={0}
              className="border rounded p-2"
              value={filters.agingOver ?? ""}
              onChange={(e) =>
                setFilters((f) => ({
                  ...f,
                  agingOver: e.target.value ? Number(e.target.value) : null,
                }))
              }
            />
          </label>
        ...
        <div className="flex flex-wrap items-center gap-2 border-t pt-3">
          ...
          />
          <button className="px-3 py-1 border rounded" onClick={saveView}>
            Save View
          </button>
          {views.map((v) => (
            <div
            ...
            >
              <button onClick={() => loadView(v.id)} className="underline">
                {v.name}
              </button>
              <button
                onClick={() => {
      ...
      >
        {rows === null ? (
          <div className="p-6 text-sm text-rose-600">
            Simulated error: gagal memuat data.
          </div>
        ) : rows.length === 0 ? (
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {rows.map((a) => (
                <tr
                  key={a.id}
                  className="hover:bg-slate-50 focus-within:bg-slate-100 cursor-pointer"
                  tabIndex={0}
                  onClick={() => AppRouter.navigate(`/applications/${a.id}`)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") {
                      e.preventDefault();
                      AppRouter.navigate(`/applications/${a.id}`);
                    }
                  }}
                >
                  ...
                  <td className="p-2 text-xs">
                    {a.flags.duplicate_face ? "👤dup " : ""}
                    {a.flags.duplicate_nik ? "🆔dup " : ""}
                    {a.flags.device_anomaly ? "📱anom" : ""}
                  </td>
                  <td className="p-2">
                    {a.assigned_to
                      ? (usersById.get(a.assigned_to) ?? a.assigned_to)
                      : "—"}
                  </td>
