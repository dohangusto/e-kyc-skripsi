...
export default function DistributionPage() {
  ...
  const selectedBatchCodes = useMemo(
    () =>
      selectedBatchIds
        .map((id) => batchById.get(id)?.code)
        .filter((code): code is string => !!code),
    [batchById, selectedBatchIds],
  );

  const effectiveBeneficiaries = selectedBatchIds.length
    ? selectedBatchBeneficiaries
  ...
  async function advanceStatus(
    ...
    next: Distribution["status"],
  ) {
    try {
      if (!session) throw new Error("Harus login sebagai admin");
      await Data.updateDistributionStatus(
        distribution.id,
        next,
        session.userId,
      );
  ...
  async function notify(distribution: Distribution, ids: string[]) {
    try {
      ...
      Toast.show(`Notifikasi template dikirim ke ${pending.length} penerima`);
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      ...
      <PageIntro>
        Susun jadwal penyaluran, pilih batch atau penerima tertentu, dan pantau
        progres notifikasi.
      </PageIntro>
      ...
      <section className="bg-white border rounded p-4 space-y-3">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Pilih batch atau penerima yang sudah siap disalurkan, lalu tentukan
            jadwal penyaluran.
          </p>
        </div>
          ...
          <label className="text-sm space-y-1">
            ...
            <select
              ...
              onChange={(e) =>
                setForm((prev) => ({
                  ...prev,
                  channel: e.target.value as Distribution["channel"],
                }))
              }
            >
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between gap-2">
            <h4 className="font-medium text-sm">
              Batch Siap Disalurkan ({batches.length})
            </h4>
          ...
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              ...
              >
                ...
                <span>
                  ...
                  <span className="block text-slate-500">
                    {batch.items.length} penerima · Status {batch.status}
                  </span>
                </span>
              </label>
            ))}
            {batches.length === 0 && (
              <p className="text-sm text-slate-500">
                Belum ada batch simulasi.
              </p>
            )}
          </div>
          {selectedBatchIds.length > 0 && (
            <div className="text-xs text-slate-600">
              Menyalurkan {selectedBatchBeneficiaries.length} penerima dari
              batch terpilih.
            </div>
          )}
        </div>
        ...
        {selectedBatchIds.length === 0 && (
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-2">
              <h4 className="font-medium text-sm">
                Penerima Siap Salur ({candidates.length})
              </h4>
              <button
                className="text-xs text-slate-500 underline"
                onClick={() =>
                  setSelectedBeneficiaries(candidates.map((app) => app.id))
                }
              >
                Pilih semua
              </button>
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {candidates.map((app) => (
                <label
                  key={app.id}
                  className="flex items-start gap-2 border rounded p-2 text-xs"
                >
                  <input
                    type="checkbox"
                    className="mt-1"
              ...
              {candidates.length === 0 && (
                <p className="text-sm text-slate-500">
                  Belum ada penerima yang siap salur.
                </p>
              )}
            </div>
          </div>
        ...
        <div className="flex justify-end">
          <RoleGate allow={["ADMIN"]}>
            <button
              className="px-4 py-2 border rounded bg-slate-900 text-white text-sm"
      ...
      <section className="bg-white border rounded p-4 space-y-4">
        ...
        <div className="space-y-3">
          {distributions.map((distribution) => {
            const pending = distribution.beneficiaries.filter(
              (id) => !distribution.notified.includes(id),
            );
            return (
              <article
                key={distribution.id}
              ...
              >
                <header className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="text-xs text-slate-500">
                      {formatDate(distribution.scheduled_at)} ·{" "}
                      {channelLabel(distribution.channel)} ·{" "}
                      {distribution.location}
                    </div>
                    <div className="text-xs text-slate-500">
                      Batch terkait:{" "}
                      {distribution.batch_codes.length
                        ? distribution.batch_codes.join(", ")
                        : "—"}
                    </div>
                  </div>
                  <span
                    className={`px-2 py-1 rounded text-xs font-medium ${STATUS_CLASS[distribution.status]}`}
                  >
                ...
                <div className="bg-blue-50 border border-blue-100 rounded p-3 text-xs text-slate-700">
                  <div className="font-semibold text-blue-800 text-sm">
                    Pesan otomatis ke penerima
                  </div>
                  <p className="mt-1">
                    {distributionTemplatePreview(distribution)}
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  <h4 className="font-medium text-xs uppercase tracking-wide text-slate-500">
                    Daftar penerima
                  </h4>
          ...
          {distributions.length === 0 && (
            <p className="text-sm text-slate-500">
              Belum ada jadwal penyaluran yang tersimpan.
            </p>
          )}
        </div>
      </section>
...
function distributionTemplatePreview(distribution: Distribution) {
  const when = formatDate(distribution.scheduled_at);
  const channel = channelLabel(distribution.channel);
  const batches = distribution.batch_codes.length
    ? `Batch: ${distribution.batch_codes.join(", ")}`
    : "";
  const parts = [
    `Penyaluran ${distribution.name} pada ${when} via ${channel}`,
    distribution.location,
    batches,
  ].filter(Boolean);
  return parts.join(" · ");
    ...
    <li className="border rounded p-2 flex flex-wrap items-center justify-between gap-2 text-xs">
      <div>
        <div className="font-semibold text-slate-700">
          {app?.applicant.name ?? "Tidak ditemukan"}
        </div>
        <div className="text-slate-500">
          {id} {app ? `· ${app.region.kec}` : ""}{" "}
