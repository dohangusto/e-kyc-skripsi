...
export default function OverviewPage() {
  ...
  const inReview = accessible.filter(
    (a) => a.status === "DESK_REVIEW" || a.status === "FIELD_VISIT",
  ).length;
  const approved = accessible.filter(
    (a) => a.status === "FINAL_APPROVED",
  ).length;
  const rejected = accessible.filter(
    (a) => a.status === "FINAL_REJECTED",
  ).length;
  const disbReady = accessible.filter(
    (a) => a.status === "DISBURSEMENT_READY",
  ).length;
  const flagged = accessible.filter(
    (a) =>
      a.flags.duplicate_face || a.flags.duplicate_nik || a.flags.device_anomaly,
  ).length;
  const aging = accessible.filter(
    (a) =>
      a.aging_days > 3 &&
      (!a.visits.length || a.visits.some((v) => v.status !== "VERIFIED")),
  ).length;
  const surveyCompleted = accessible.filter((a) => a.survey?.completed).length;
  const surveyPending = Math.max(total - surveyCompleted, 0);
  const surveyCompletionRatio =
    total === 0 ? 0 : Math.round((surveyCompleted / total) * 100);
  ...
  const funnelData = useMemo(() => {
    ...
    return [
      {
        label: "Submitted",
        count: accessible.filter((a) =>
          ["SUBMITTED", "RETURNED_FOR_REVISION"].includes(a.status),
        ).length,
      },
      {
        label: "Desk Review",
        count: accessible.filter((a) => a.status === "DESK_REVIEW").length,
      },
      {
        label: "Field Visit",
        count: accessible.filter((a) => a.status === "FIELD_VISIT").length,
      },
      {
        label: "Final",
        count: accessible.filter((a) => finalStatuses.has(a.status)).length,
      },
    ];
  }, [accessible]);
  ...
  const batchCounts = useMemo(
    () =>
      computeStatusCount(
        snapshot.batches.map((b) => b.status),
        BATCH_FILTERS,
      ),
    [snapshot.batches],
  );
  const distributionCounts = useMemo(
    () =>
      computeStatusCount(
        snapshot.distributions.map((d) => d.status),
  ...
  const filteredBatches = useMemo(() => {
    ...
    if (batchFilter !== "ALL")
      items = items.filter((batch) => batch.status === batchFilter);
    return items.slice(0, 6);
  }, [batchFilter, snapshot.batches]);
  const filteredDistributions = useMemo(() => {
    let items = snapshot.distributions.slice();
    if (distributionFilter !== "ALL")
      items = items.filter((dist) => dist.status === distributionFilter);
    return items.slice(0, 6);
  ...
  return (
    <div className="space-y-4">
      ...
      <PageIntro>
        Pantau indikator utama pengajuan, kunjungan lapangan, dan penyaluran
        dalam satu layar.
      </PageIntro>
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
        ...
        <Card
          ...
          value={flagged}
          tone="warning"
          helper="Cek ulang sebelum lanjut"
        />
        <Card
          title="Survey Selesai"
          value={surveyCompleted}
          helper={`${surveyCompletionRatio}% terisi`}
          tone="success"
        />
        <Card
          title="Survey Pending"
          value={surveyPending}
          helper={total ? `${surveyPending} dari ${total}` : "—"}
        />
      </div>
      <section className="bg-white border rounded p-4">
        ...
        <div className="flex flex-wrap gap-3 text-sm">
          <button
            ...
            onClick={() => AppRouter.navigate("/applications?flagged=1")}
          >
            Flagged ({flagged})
          </button>
          <button
            ...
            onClick={() =>
              AppRouter.navigate(
                "/applications?status=FIELD_VISIT&status=DESK_REVIEW&aging=3",
              )
            }
          >
            {agingLabel}
      ...
      {(session?.role === "TKSK" || session?.role === "AUDITOR") && (
...
function PieChart({
  ...
  data.forEach((item) => {
    ...
    const end = acc;
    segments.push(
      `${item.color} ${Math.round(start * 100)}% ${Math.round(end * 100)}%`,
    );
  });
    ...
    <div className="bg-white border rounded p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          onClick={() => AppRouter.navigate("/tksk")}
        >
          Kelola TKSK
        </button>
      </div>
      <p className="text-xs text-slate-500">
        Daftar kunjungan lapangan yang akan berlangsung satu minggu ke depan.
      </p>
      {visits.length === 0 ? (
        <p className="text-sm text-slate-500">
          Belum ada kunjungan dalam waktu dekat.
        </p>
      ) : (
        <ul className="space-y-2 text-sm">
          {visits.map((item) => (
            <li
              key={`${item.appId}-${item.scheduledAt}`}
            ...
            >
              ...
              <div className="text-right text-xs text-slate-500">
                ...
                <span className="inline-block mt-1 px-2 py-0.5 rounded bg-blue-100 text-blue-600">
                  {item.status}
                </span>
              </div>
            </li>
  ...
  return (
    <div className="bg-white border rounded p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          className="text-xs text-blue-600 underline"
          onClick={() => AppRouter.navigate("/batches")}
        >
          Kelola batch
        </button>
      </div>
            ...
            <li key={batch.id} className="border rounded p-3 space-y-1">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-slate-500">
                    ID {batch.id} · {batch.items.length} penerima
                  </div>
                </div>
  ...
  return (
    <div className="bg-white border rounded p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          className="text-xs text-blue-600 underline"
          onClick={() => AppRouter.navigate("/distribution")}
        >
          Kelola penyaluran
        </button>
      </div>
      ...
      {items.length === 0 ? (
        <p className="text-sm text-slate-500">
          Tidak ada penyaluran untuk filter ini.
        </p>
      ) : (
        <ul className="space-y-2 text-sm">
          {items.map((dist) => (
            <li key={dist.id} className="border rounded p-3 space-y-1">
              <div className="flex items-center justify-between gap-2">
                <div>
                  ...
                  <div className="text-xs text-slate-500">
                    {new Date(dist.scheduled_at).toLocaleString("id-ID", {
                      dateStyle: "medium",
                      timeStyle: "short",
                    })}
                  </div>
              ...
              <div className="text-xs text-slate-500">
                <span className="inline-block px-2 py-0.5 rounded bg-slate-100 text-slate-600">
                  {dist.channel}
                </span>
                <span className="ml-2">
                  {dist.beneficiaries.length} penerima • {dist.notified.length}{" "}
                  diberi tahu
                </span>
              </div>
              {dist.batch_codes.length > 0 && (
                <div className="text-xs text-slate-500">
                  Batch: {dist.batch_codes.join(", ")}
                </div>
              )}
            </li>
          ))}
...
function computeStatusCount<T extends string>(
  values: T[],
  filters: readonly (T | "ALL")[],
): Record<T | "ALL", number> {
