...
func (h *BackofficeHTTPHandler) EscalateApplication(c echo.Context) error {
	id := c.Param("id")
	var req struct {
		Actor  string `json:"actor"`
		Reason string `json:"reason"`
	...
	if err := h.Service.EscalateApplication(c.Request().Context(), id, req.Actor, req.Reason); err != nil {
		if errors.Is(err, domain.ErrNotFound) {
			return respondError(c, http.StatusNotFound, err)
		}
		return respondError(c, http.StatusInternalServerError, err)
...
func (h *BackofficeHTTPHandler) ConfirmDuplicate(c echo.Context) error {
	id := c.Param("id")
	var req struct {
	...
	if req.CandidateID == "" || req.Actor == "" {
		return respondError(c, http.StatusBadRequest, errors.New("candidateId and actor required"))
	}
	if err := h.Service.ConfirmDuplicate(c.Request().Context(), id, req.CandidateID, req.Actor, req.Note); err != nil {
		if errors.Is(err, domain.ErrNotFound) {
			return respondError(c, http.StatusNotFound, err)
		}
		return respondError(c, http.StatusInternalServerError, err)
...
func (h *BackofficeHTTPHandler) IgnoreDuplicate(c echo.Context) error {
	id := c.Param("id")
	var req struct {
		Actor string `json:"actor"`
		Note  string `json:"note"`
	...
	if err := h.Service.IgnoreDuplicate(c.Request().Context(), id, req.Actor, req.Note); err != nil {
		if errors.Is(err, domain.ErrNotFound) {
			return respondError(c, http.StatusNotFound, err)
		}
		return respondError(c, http.StatusInternalServerError, err)
